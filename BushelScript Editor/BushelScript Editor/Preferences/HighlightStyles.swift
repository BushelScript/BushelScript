...
private var highlightStylesCache = Cache<URL, Cache<CGFloat, HighlightStyles>>()

func fetchOrMakeHighlightStyles(fontSize: CGFloat? = nil) throws -> HighlightStyles {
    let fontSize = fontSize ?? Defaults[.sourceCodeFont].pointSize
    
    guard let themeFile = try themeFile(for: Defaults[.themeFileName]) else {
        return HighlightStyles()
    }
    
    return try highlightStylesCache
        .for(themeFile, default: Cache())
        .for(fontSize, default: {
        let plist = try Data(contentsOf: themeFile)
        let theme = try PropertyListDecoder().decode(Theme.self, from: plist)
        
        let fontProvider = FontProvider(size: fontSize)
