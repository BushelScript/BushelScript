...
public func propagate<Result>(up moduleStack: RT_ModuleStack, _ function: (_ target: RT_Object) throws -> Result?) rethrows -> Result? {
    try moduleStack.contents.reversed().reduce(nil) { try $0 ?? function($1) }
}
