...
public final class RT_TestSpecifier: RT_Object {
    ...
    public func saTestClause(appData: AppData) -> SwiftAutomation.TestClause? {
        ...
        func makeComparisonTestClause() -> SwiftAutomation.TestClause? {
            ...
            let other: RT_Object
            let reverse: Bool
            if let lhsObjectSpecifier = (lhs as? RT_AESpecifierProtocol)?.saSpecifier(appData: appData) as? SwiftAutomation.ObjectSpecifier {
                objectSpecifier = lhsObjectSpecifier
                other = rhs
                reverse = false
            } else {
                guard let rhsObjectSpecifier = (rhs as? RT_AESpecifierProtocol)?.saSpecifier(appData: appData) as? SwiftAutomation.ObjectSpecifier else {
                    return nil
                }
