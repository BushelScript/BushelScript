...
public protocol AEEncodable {
    func encodeAEDescriptor(_ appData: AppData) throws -> NSAppleEventDescriptor
}

public protocol AEDecodable {
    init(from descriptor: NSAppleEventDescriptor, appData: AppData) throws
...
public enum MayBeMissing<T>: AECodable { // TO DO: rename 'MissingOr<T>'? this'd be more in keeping with TypeSupportSpec-generated enum names (e.g. 'IntOrStringOrMissing')
    ...
    public init(from descriptor: NSAppleEventDescriptor, appData: AppData) throws {
        if isMissingValue(descriptor) {
            ...
            self = .value(try appData.unpack(descriptor) as T)
        }
    }
...
extension Optional: AECodable {
    ...
    public init(from descriptor: NSAppleEventDescriptor, appData: AppData) throws {
        if isMissingValue(descriptor) {
            ...
            self = .some(try appData.unpack(descriptor))
        }
    }
...
extension Set: AECodable { // note: AEM doesn't define a standard AE type for Sets, so pack/unpack as typeAEList (we'll assume client code has its own reasons for suppling/requesting Set<T> instead of Array<T>)
    ...
    public init(from descriptor: NSAppleEventDescriptor, appData: AppData) throws {
        ...
        case _typeAEList:
            for i in 1..<(descriptor.numberOfItems+1) { // bug workaround for zero-length range: 1...0 throws error, but 1..<1 doesn't
                do {
                    result.insert(try appData.unpack(descriptor.atIndex(i)!) as Element)
                } catch {
                    throw UnpackError(appData: appData, descriptor: descriptor, type: Self.self, message: "Can't unpack item \(i) as \(Element.self).")
                }
            }
        default:
            result.insert(try appData.unpack(descriptor) as Element)
        }
        self = result
    }
...
extension Array: AECodable {
    ...
    private static func unpackInt16Array(_ desc: NSAppleEventDescriptor, appData: AppData, indexes: [Int]) throws -> [Element] {
        if Element.self == Int.self { // common case
            ...
            return try self.init(from: try appData.pack(appData.unpack(desc) as [Int]), appData: appData)
        }
    }
    ...
    public init(from descriptor: NSAppleEventDescriptor, appData: AppData) throws {
        ...
        case _typeAEList:
            ...
            for i in 1..<(descriptor.numberOfItems+1) { // bug workaround for zero-length range: 1...0 throws error, but 1..<1 doesn't
                do {
                    result.append(try appData.unpack(descriptor.atIndex(i)!) as Element)
                } catch {
                    throw UnpackError(appData: appData, descriptor: descriptor, type: Self.self, message: "Can't unpack item \(i) as \(Element.self).")
                }
            }
        ...
        case _typeQDPoint: // SInt16[2]
            self = try Array<Element>.unpackInt16Array(descriptor, appData: appData, indexes: [1,0]) // QDPoint is YX; swap to give [X,Y]
        case _typeQDRectangle: // SInt16[4]
            self = try Array<Element>.unpackInt16Array(descriptor, appData: appData, indexes: [1,0,3,2]) // QDPoint is Y0X0Y1X1; swap to give [X0,Y0,X1,Y1]
        case _typeRGBColor: // UInt16[3] (used by older Carbon apps; Cocoa apps use lists)
            self = try Array<Element>.unpackUInt16Array(descriptor, appData: appData, indexes: [0,1,2])
        default:
            self = [try appData.unpack(descriptor) as Element]
        }
    }
...
extension Dictionary: AECodable {
    ...
    public init(from descriptor: NSAppleEventDescriptor, appData: AppData) throws {
        if !descriptor.isRecordDescriptor {
            throw UnpackError(appData: appData, descriptor: descriptor, type: Self.self, message: "Not a record.")
        }
