...
extension AppData {
    ...
    public func unpack<T>(_ desc: NSAppleEventDescriptor) throws -> T {
        if T.self == Any.self || T.self == AnyObject.self {
            ...
            return try t.init(from: desc, appData: self) as! T
        } else if T.self == Query.self {
            // note: user typically specifies exact class ([PREFIX][Object|Elements]Specifier)
    ...
    public func unpackAsAny(_ desc: NSAppleEventDescriptor) throws -> Any { // note: this never returns Optionals (i.e. cMissingValue AEDescs always unpack as MissingValue when return type is Any) to avoid dropping user into Optional<T>.some(Optional<U>.none) hell.
        ...
        case _typeLongDateTime:
            ...
            return result
        case _typeAEList:
            return try Array(from: desc, appData: self) as [Any]
        case _typeAERecord:
            return try Dictionary(from: desc, appData: self) as [Symbol:Any]
        ...
        if let operatorType = desc.forKeyword(_keyAECompOperator),
            ...
            !_comparisonOperatorCodes.contains(operatorType.enumCodeValue) {
                ...
                if operatorType.typeCodeValue == _kAEContains && !(operand1 is ObjectSpecifier) {
                    ...
                    return ComparisonTest(operatorType: operatorType, operand1: op1, operand2: operand2, appData: self)
                } // else fall through to throw
        }
    ...
    public func sendAppleEvent<T>(name: String?, eventClass: OSType, eventID: OSType,
        ...
        do {
            ...
            if let result = MissingValue as? T { // this will succeed when T is Any (which it always will be when the caller ignores the command's result)
                return result
            } else if let t = T.self as? AEDecodable.Type { // cover the crusty Carbon app case in a type-safe way (e.g. if the command usually returns a list, the caller will naturally expect it _always_ to return one so T will be Array<>, in which case return an empty array; OTOH, if the command usually returns a string, the user will _have_ to specify MayBeMissing<String>/Optional<String> or else they'll get an UnpackError)
...
extension AppData {
    ...
    public func doTransaction<T>(session: Any? = nil, closure: () throws -> (T)) throws -> T {
        ...
        self._transactionID = try self.sendAppleEvent(name: nil, eventClass: _kAEMiscStandards, eventID: _kAEBeginTransaction, parentSpecifier: AEApp, directParameter: session as Any) as AETransactionID
        defer {
